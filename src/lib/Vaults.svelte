...
<div class="wrapper">
  ...
  <div class="header">
    ...
    <button
      onclick={onCross}
      title={createVault || unlockAtmpt !== -1 ? "Go Back" : "Create Vault"}
    >
      <span class={createVault || unlockAtmpt !== -1 ? "plus cross" : "plus"}
      ></span>
    </button>
  </div>
  <div class="vaults">
    ...
    {:else}
      <div class="create-vault">
        <h3>Create Vault</h3>
        <Form inputs={INPUTS} SubmitBtn="Create" onSubmit={onCreateVault} />
      </div>
    {/if}
  </div>
</div>
