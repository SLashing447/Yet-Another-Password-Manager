<script lang="ts">
    ...
    const minimize = async () => await Operator.minimizeWindow();
    const close = async () => await Operator.closeWindow();
    const maxm = async () => {
        if (isMaximized) {
            await Operator.unmaximizeWindow();
        } else {
            await Operator.maximizeWindow();
        }
